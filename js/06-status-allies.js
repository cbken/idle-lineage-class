        ...
        try {
            _withAllyEquipmentContext(ctx.source, () => {
                if (banked > 0) {
                    ...
                    while ((ctx.source.lv || 1) < 200 && ctx.source.exp >= getExpReq(ctx.source.lv)) {
                        ctx.source.exp -= getExpReq(ctx.source.lv);
                        ctx.source.lv++;
                        if (ctx.source.lv >= 50) ctx.source.bonus = (ctx.source.bonus || 0) + 1;
                    }
                    if ((ctx.source.lv || 1) >= 200) ctx.source.exp = 0;
                }
                if (alignmentDelta) {
...
function mercExpClaimPending(_retry) {
    try {
        ...
        if (total > 0) {
            ...
            while ((player.lv || 1) < 200 && player.exp >= getExpReq(player.lv)) { player.exp -= getExpReq(player.lv); player.lv++; if (player.lv >= 50) player.bonus = (player.bonus || 0) + 1; }   // 比照 checkLvUp 升級曲線
            if ((player.lv || 1) >= 200) player.exp = 0;   // 滿等不留溢出經驗
        }
